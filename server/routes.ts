...
import {
  ...
  DEFAULT_CLIENT_NUTRITION_LIMITS,
...
function mergeLimitSet(
  current: ClientPersonalizationSettings["nutritionLimits"]["daily"],
  incoming: Partial<ClientPersonalizationSettings["nutritionLimits"]["daily"]> | undefined,
) {
  if (!incoming) return current;
  return {
    calories: incoming.calories !== undefined ? normalizeLimitValue(incoming.calories) : current.calories,
    protein: incoming.protein !== undefined ? normalizeLimitValue(incoming.protein) : current.protein,
    carbs: incoming.carbs !== undefined ? normalizeLimitValue(incoming.carbs) : current.carbs,
    fat: incoming.fat !== undefined ? normalizeLimitValue(incoming.fat) : current.fat,
    fiber: incoming.fiber !== undefined ? normalizeLimitValue(incoming.fiber) : current.fiber,
    sugar: incoming.sugar !== undefined ? normalizeLimitValue(incoming.sugar) : current.sugar,
    sodium: incoming.sodium !== undefined ? normalizeLimitValue(incoming.sodium) : current.sodium,
    cholesterol: incoming.cholesterol !== undefined ? normalizeLimitValue(incoming.cholesterol) : current.cholesterol,
  };
}
...
function getOrCreateClientPersonalizationSettings(userId: string): ClientPersonalizationSettings {
  ...
  if (existing) {
    const normalized: ClientPersonalizationSettings = {
      ...createDefaultClientPersonalizationSettings(userId),
      ...existing,
      profile: {
        ...createDefaultClientPersonalizationSettings(userId).profile,
        ...(existing.profile || {}),
      },
      formulas: {
        ...createDefaultClientPersonalizationSettings(userId).formulas,
        ...(existing.formulas || {}),
      },
      nutritionLimits: {
        daily: {
          ...DEFAULT_CLIENT_NUTRITION_LIMITS.daily,
          ...(existing.nutritionLimits?.daily || {}),
        },
        perMeal: {
          ...DEFAULT_CLIENT_NUTRITION_LIMITS.perMeal,
          ...(existing.nutritionLimits?.perMeal || {}),
        },
      },
    };
...
function mergeClientPersonalizationSettings(
  ...
  const incomingLimits = (incoming.nutritionLimits || {}) as Partial<ClientPersonalizationSettings["nutritionLimits"]>;
  ...
  return {
    ...
    nutritionLimits: {
      daily: mergeLimitSet(current.nutritionLimits.daily, incomingLimits.daily),
      perMeal: mergeLimitSet(current.nutritionLimits.perMeal, incomingLimits.perMeal),
    },
  };
}
...
export async function registerRoutes(
  ...
  const buildRecallPayload = (
    mealsList: any[],
    medicationsList: any[],
    settings: ClientPersonalizationSettings,
    medicalInfo: { conditions?: string | null; lastMeal?: string | null; symptoms?: string | null; medications?: string | null } | null,
  ) => {
    ...
    const lastMeal = sortedMeals.length > 0 ? sortedMeals[0] : null;
    ...
    const inferredLikes = Array.from(mealCounts.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 6)
      .map(([name]) => name);

    const likes = normalizeStringList([
      ...(settings.profile.favoriteFoodsAdult || []),
      ...(settings.profile.favoriteFoodsKids || []),
    ]);

    const dislikes = normalizeStringList([
      ...(settings.profile.avoidFoods || []),
      ...(settings.profile.disabledFoodNames || []),
    ]);

    const conditions = normalizeStringList([
      ...(settings.profile.conditions || []),
      ...splitCsvList(medicalInfo?.conditions),
    ]);
    ...
    return {
      lastMeal: lastMeal
        ? {
            ...
            name: lastMeal.name,
            date: lastMeal.date,
            calories: lastMeal.calories ?? null,
            protein: lastMeal.protein ?? null,
            carbs: lastMeal.carbs ?? null,
            fat: lastMeal.fat ?? null,
            fiber: lastMeal.fiber ?? null,
            sugar: lastMeal.sugar ?? null,
            sodium: lastMeal.sodium ?? null,
            cholesterol: lastMeal.cholesterol ?? null,
          }
        : medicalInfo?.lastMeal
          ? {
              id: null,
              name: medicalInfo.lastMeal,
              date: null,
              calories: null,
              protein: null,
              carbs: null,
      ...
      medicalMedications: splitCsvList(medicalInfo?.medications),
    };
  };
  ...
  app.post("/api/auth/register", async (req: Request, res: Response) => {
    try {
      ...
      const allergyFoods = normalizeListInput(profilePayload.allergyFoods || profilePayload.allergies || profilePayload.allergyFood || profilePayload.allergyItems);
      const favoriteFoods = normalizeListInput(profilePayload.favoriteFoods || profilePayload.favoriteFood || profilePayload.likes);
      const conditions = normalizeListInput(profilePayload.conditions || profilePayload.diseases || profilePayload.illnesses);
      const symptoms = normalizeListInput(profilePayload.symptoms || profilePayload.issues);
      const medications = normalizeListInput(profilePayload.medications || profilePayload.drugs);

      const currentSettings = getOrCreateClientPersonalizationSettings(newUser.id);
      saveClientPersonalizationSettings({
        ...currentSettings,
        profile: {
          ...currentSettings.profile,
          allergies: allergyFoods.length ? allergyFoods : currentSettings.profile.allergies,
          favoriteFoodsAdult: favoriteFoods.length ? favoriteFoods : currentSettings.profile.favoriteFoodsAdult,
          conditions: conditions.length ? conditions : currentSettings.profile.conditions,
        },
        updatedAt: new Date().toISOString(),
      });

      await upsertDietaryPreferenceByUserId(newUser.id, {
        otherAllergies: allergyFoods.join(", ") || null,
        favoriteFoods: favoriteFoods.join(", ") || null,
      });

      await upsertEmergencyMedicalInfoByUserId(newUser.id, {
        bloodType: bloodType || null,
        allergies: allergyFoods.join(", ") || null,
        lastMeal: profilePayload.lastMeal ? String(profilePayload.lastMeal).trim() : null,
        symptoms: symptoms.join(", ") || null,
        medications: medications.join(", ") || null,
  ...
  // POST /api/auth/subscribe - Activate the user's subscription immediately
  app.post("/api/auth/subscribe", async (req: Request, res: Response) => {
    try {
      ...
      const plan = req.body?.plan === "annual" ? "annual" : "trial_15";
      const durationDays = plan === "annual" ? 365 : 15;
      ...
      const { password, ...userWithoutPassword } = (updated || user) as any;
    ...
    try {
      ...
      const mealsWithNutrition = mealsList.map((m: any) => {
        ...
        return {
          ...
          fiber: n ? Number(n.fiber) : 0,
          sugar: n ? Number(n.sugar) : 0,
          sodium: n ? Number(n.sodium) : 0,
          cholesterol: n ? Number(n.cholesterol) : 0,
        };
      });
  ...
  app.post("/api/nutrition/meals", async (req: Request, res: Response) => {
    try {
      ...
      const { calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol } = req.body;
      if (calories != null) {
        await db!.insert(nutrients).values({
          ...
          fiber: fiber != null ? String(Number(fiber)) : null,
          sugar: sugar != null ? String(Number(sugar)) : null,
          sodium: sodium != null ? String(Number(sodium)) : null,
          cholesterol: cholesterol != null ? String(Number(cholesterol)) : null,
        });
      }

      res.status(201).json({ ...newMeal[0], calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol });
    } catch (error) {
  ...
  app.put("/api/nutrition/meals/:id", async (req: Request, res: Response) => {
    try {
      ...
      res.json({ ...updated[0], calories, protein, carbs, fat, fiber, sugar, sodium, cholesterol });
    } catch (error) {
    ...
    try {
      ...
      if (isMockMode) {
        ...
        for (const meal of userMeals) {
          ...
          summary.totalFiber += Number((meal as any).fiber || 0);
          summary.totalSugar += Number((meal as any).sugar || 0);
          summary.totalSodium += Number((meal as any).sodium || 0);
          summary.totalCholesterol += Number((meal as any).cholesterol || 0);
        }
      ...
      for (const meal of mealsList as any[]) {
        for (const nutrient of (meal.nutrients ?? [])) {
          ...
          summary.totalFiber += Number(nutrient.fiber || 0);
          summary.totalSugar += Number(nutrient.sugar || 0);
          summary.totalSodium += Number(nutrient.sodium || 0);
          summary.totalCholesterol += Number(nutrient.cholesterol || 0);
        }
      }
  ...
  app.get("/api/admin/users/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      if (isMockMode) {
        ...
        const userMedical      = mockEmergencyMedInfo.get(uid) || null;
        const recall           = buildRecallPayload(userMeals, userMeds, getOrCreateClientPersonalizationSettings(uid), userMedical as any);
        ...
        return res.json({
          ...
          emergencyMedicalInfo: userMedical,
          recall,
      ...
      const recall = buildRecallPayload(userMeals as any[], userMeds as any[], getOrCreateClientPersonalizationSettings(uid), await getEmergencyMedicalInfoByUserId(uid) as any);
      const { password: _, ...safe } = u;
      res.json({
        ...safe,
        role: getUserRole(u),
